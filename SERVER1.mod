		...
		frameMutex:= TRUE;
            ...
            CASE 96: 
                IF nParams = 2 THEN
                    p1 := params_str{1};
                    p2 := params_str{2};
                    IF p1 = "do_0" AND p2 = "1" THEN
                        Set do_0;
                    ELSEIF p1 = "do_0" AND p2 = "0" THEN
                        Reset do_0;
                    ENDIF
                    ok := SERVER_OK;
                ELSE
            ...
            CASE 97: 
                IF nParams = 2 THEN
                    p1 := params_str{1};
                    p2 := params_str{2};
                    IF p1 = "di_0" AND p2 = "1" THEN
                        WaitUntil di_0=1;
                    ELSEIF p1="di_0" AND p2="0" THEN
                        WaitUntil di_0=0;
                    ENDIF 
                    ok := SERVER_OK;
                ELSE
...
ENDPROC
PROC GripperOFF()
   RESET DO_00_GripperON;
   WaitTime 0.3;
   Set DO_01_GripperOFF;
   WaitTime 0.5;
   WaitUntil DI_00_sensor_Open=1; 
ENDPROC
PROC GripperON()
   RESET DO_01_GripperOFF;
   WaitTime 0.3;
   Set DO_00_GripperON;
   WaitTime 0.5;
   WaitUntil DI_01_sensor_Close=1;
ENDPROC
ENDMODULE
